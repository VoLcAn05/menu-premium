...
export function CartSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/80 backdrop-blur-sm sm:items-center sm:p-4">
      ...
      <div className="animate-fade-in-up relative flex max-h-[85vh] w-full flex-col overflow-hidden rounded-t-2xl bg-charcoal shadow-premium sm:max-w-md sm:rounded-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {items.length === 0 ? (
            <p className="py-10 text-center text-sm text-cream/50">
              Todavía no has agregado nada de la carta.
            </p>
          ) : (
        ...
        <div className="border-t border-cream/10 px-5 py-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="font-display text-xl font-normal text-cream">
              {formatUSD(total)}
            </span>
          </div>
          <button
            ...
            className="w-full rounded-full bg-gold py-3.5 text-center text-sm font-semibold text-ink shadow-premium transition-transform disabled:cursor-not-allowed disabled:opacity-40 active:scale-[0.98]"
          >
            {submitting
